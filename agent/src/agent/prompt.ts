...
export const RESPONSE_POLICY = [
  ...
  "- When you screen with `rank_universe` and then backtest the chosen coins over an overlapping window, you MUST pass `--as-of` equal to the backtest start date (or earlier) to anchor the screening features. Forgetting this leaks the backtest outcome into the screen and is the most common source of inflated `return` / `max_upside` results.",
