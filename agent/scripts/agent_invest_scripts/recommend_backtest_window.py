...
from agent_invest_scripts._lib.backtest.benchmarks import benchmark_coin_ids
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--benchmark-objective",
        choices=["high_growth", "balanced", "preserve_capital", "income"],
        help=(
            "Keep the window inside this objective's benchmark data "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        with script_timeout(resolve_timeout_seconds(args.timeout_seconds)):
            bench_ids = (
                list(benchmark_coin_ids(args.benchmark_objective))
                if args.benchmark_objective
                else None
            )
