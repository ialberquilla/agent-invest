...
def test_reads_dataset_from_storage_root(tmp_path: Path, monkeypatch) -> None:
    ...
    first = data.read_dataset("universe_history")
    second = data.read_dataset("universe_history")
...
def test_missing_dataset_raises_typed_error(tmp_path: Path, monkeypatch) -> None:
    ...
    try:
        data.read_dataset("daily_prices")
        ...
        raise AssertionError(
            "read_dataset should fail when the parquet file is missing"
        )
